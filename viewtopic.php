...
window.addEventListener('offline', function(e) {
  ...
  if (file_exists(__DIR__.'/data/messages/'.cleanFilename($GLOBALS['room']).'/del.json')) {
	...
	fclose($file);
	if (isset($_POST['vote']) && !isset($_POST['post'])) {
		?><div style="text-align:center;padding:10px;background-color:lightyellow;color:black;"><?php
		if (getname()) {
			$vote = $_POST['vote'];
			if ($vote == 'up') 
				$obj->votes = (isset($obj->votes) ? $obj->votes : 0) + 1;
			if ($vote == 'down')
				$obj->votes = (isset($obj->votes) ? $obj->votes : 0) - 1;

			class Vote {
				public function __construct(int $upOrDown) {
					$this->way = $upOrDown;
					$this->user = getname();
					$this->time = time();
					$this->topicName = $_GET['room'];
				}
			}

			if ($vote == 'down') {
				?>You have voted down. Be sure to add a comment with constructive criticism!<?php
			} else {
				?>Thank you for voting!<?php
			}
			if (!isset($obj->voters)) $obj->voters = array();

			$a = $_POST['vote'] == 'up' ? 1 : 0;

			array_push($obj->voters, new Vote($a));
			$v = file_exists(__DIR__ . '/data/accounts/' . cleanFilename(getname()) . '/votes.json') ? json_decode(file_get_contents(__DIR__ . '/data/accounts/' . cleanFilename(getname()) . '/votes.json')) : json_decode('{"votes":[]}');
			array_push($v->votes, new Vote($a));
			fwrite(fopen(__DIR__ . '/data/accounts/' . cleanFilename(getname()) . '/votes.json', 'w+'), json_encode($v));
			fwrite(fopen(__DIR__.'/data/messages/'.cleanFilename($GLOBALS['room']).'/config.json', 'w+'), json_encode($obj));
		} else {
			?>You must be logged in to vote.<?php
		}
		?></div><?php
	}
	if (isset($_POST['vote'], $_POST['post'])) {
		$obj1 = json_decode(file_get_contents(__DIR__ . '/data/messages/' . cleanFilename($GLOBALS['room']) . '/msg.json'));
	    $post = $_POST["post"];
		$obj = $obj1->$post;
		?><div style="text-align:center;padding:10px;background-color:lightyellow;color:black;"><?php
		if (getname()) {
			...
			array_push($obj->voters, new PostVote($a));
			$v = file_exists(__DIR__ . '/data/accounts/' . cleanFilename(getname()) . '/votes.json') ? json_decode(file_get_contents(__DIR__ . '/data/accounts/' . cleanFilename(getname()) . '/votes.json')) : json_decode('{"votes":[]}');
			array_push($v->votes, new PostVote($a));
			fwrite(fopen(__DIR__ . '/data/accounts/' . cleanFilename(getname()) . '/votes.json', 'w+'), json_encode($v));
			fwrite(fopen(__DIR__.'/data/messages/'.cleanFilename($GLOBALS['room']).'/msg.json', 'w+'), json_encode($obj1));
		} else {
			?>You must be logged in to vote.<?php
		}
		?></div><?php
